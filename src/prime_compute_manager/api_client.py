...
class PrimeAPIClient:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise ValueError(
                "No API key found. Please set PRIME_API_KEY or run 'prime login'"
            )
    ...
    def _make_request_with_retry(
        self, url: str, params: dict, max_retries: int = 3, base_delay: float = 1.0
    ) -> dict:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if e.response.status_code == 429:  # Rate limited
                    ...
                    raise RuntimeError(
                        "Unauthorized. Please run 'prime login' to authenticate."
                    )
                else:
    ...
    def get_availability(
        ...
        try:
            ...
            single_gpus = self._make_request_with_retry(
                f"{self.base_url}/api/v1/availability", params=params
            )

            # Get cluster availability with retry logic
            clusters = self._make_request_with_retry(
                f"{self.base_url}/api/v1/availability/clusters", params=params
            )
