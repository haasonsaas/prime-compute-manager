...
def parse_multiline_resource(lines: List[List[str]]) -> Optional[Dict[str, Any]]:
    ...
    price_raw = (
        main_line[7].replace("…", "").replace("$", "") if len(main_line) > 7 else "0"
    )
...
def map_gpu_type(gpu_type_raw: str) -> str:
    ...
    # NVIDIA A100
    elif "A100" in gpu_type_upper or (
        gpu_type_upper.startswith("A1") and not gpu_type_upper.startswith("A10")
    ):
        if "80" in gpu_type_upper:
            return "A100_80GB"
        elif "40" in gpu_type_upper:
    ...
    # NVIDIA A6000
    elif "A6000" in gpu_type_upper or (
        gpu_type_upper.startswith("A6") and "48" in gpu_type_upper
    ):
        return "RTX_A6000"
