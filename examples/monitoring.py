...
def main():
    ...
    for i in range(3):
        try:
            pod = manager.create_pod(
                gpu_type="H100_80GB", gpu_count=2, name=f"monitoring-test-pod-{i + 1}"
            )
    ...
    try:
        ...
        if history:
            ...
            print(
                f"     Cost/hour: ${earliest.current_cost_per_hour:.2f} → "
                f"${latest.current_cost_per_hour:.2f}"
            )
            print(f"     Active pods: {earliest.active_pods} → {latest.active_pods}")
            print(
                f"     GPU usage: {earliest.total_gpus_used} → {latest.total_gpus_used}"
            )
    ...
    for pod in all_test_pods:
        try:
            ...
            print(
                f"   {final_pod.name}: ${final_pod.total_cost:.2f} "
